...
export default function TestimonialsSection() {
  ...
  const renderStars = (rating) => {
    return Array.from({ length: 5 }, (_, i) => (
      <Star key={i} className={`w-4 h-4 ${i < rating ? 'text-[#E8A87C] fill-[#E8A87C]' : 'text-gray-300'}`} />
    ));
  };

  return (
    <section className="py-24 relative overflow-hidden bg-gradient-to-br from-white via-[#F9F6F0] to-white">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-2">
        ...
        >
          <span className="text-sm font-semibold text-[#2C5F2D] tracking-wider uppercase mb-2 inline-block">Testimonials</span>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-[#0A2647] mb-6">
            What Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#2C5F2D] to-[#E8A87C]">Clients Say</span>
          </h2>
          <div className="w-24 h-1 bg-gradient-to-r from-[#2C5F2D] to-[#E8A87C] mx-auto rounded-full mb-6" />
          ...
          {!isMobile && testimonials.length > 3 && (
            <>
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={prevSlide}
                className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-16 xl:-translate-x-20 z-20 w-12 h-12 rounded-full bg-white shadow-xl flex items-center justify-center text-[#0A2647] hover:text-[#2C5F2D] transition-all duration-300 border border-[#0A2647]/10 hover:shadow-2xl"
              >
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={nextSlide}
                className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-16 xl:translate-x-20 z-20 w-12 h-12 rounded-full bg-white shadow-xl flex items-center justify-center text-[#0A2647] hover:text-[#2C5F2D] transition-all duration-300 border border-[#0A2647]/10 hover:shadow-2xl"
              >
          ...
          {isMobile && testimonials.length > 1 && (
            <>
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={prevSlide}
                className="absolute left-2 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white/90 backdrop-blur-sm shadow-lg flex items-center justify-center text-[#0A2647] hover:text-[#2C5F2D] transition-all duration-300"
              >
                <ChevronLeft className="w-5 h-5" />
              </motion.button>
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={nextSlide}
                className="absolute right-2 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white/90 backdrop-blur-sm shadow-lg flex items-center justify-center text-[#0A2647] hover:text-[#2C5F2D] transition-all duration-300"
              >
                <ChevronRight className="w-5 h-5" />
              </motion.button>
            </>
            ...
            >
              {getCurrentTestimonials().map((testimonial, idx) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-br from-[#0A2647] via-[#2C5F2D] to-[#E8A87C] opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl -z-10" />
                  <div className="absolute inset-[1px] bg-white rounded-2xl transition-all duration-500 group-hover:bg-white" />
                  
                  {/* Decorative Quote Background */}
                  <div className="absolute -top-6 -right-6 text-[#0A2647]/5 group-hover:text-[#2C5F2D]/10 transition-all duration-500">
                    ...
                    <div className="flex items-center gap-4 pt-5 border-t border-gray-100 group-hover:border-gray-200 transition-all">
                      ...
                      <div className="relative">
                        <motion.div 
                          ...
                          className="w-14 h-14 rounded-full bg-gradient-to-br from-[#0A2647] to-[#2C5F2D] p-[2px] shadow-md"
                        >
                          <img
                        ...
                        <motion.div 
                          ...
                          className="absolute -bottom-1 -right-1 w-6 h-6 bg-gradient-to-br from-[#2C5F2D] to-[#E8A87C] rounded-full flex items-center justify-center shadow-sm"
                        >
                      ...
                      {/* Client Details */}
                      <div>
                        <h4 className="font-bold text-lg text-[#0A2647] group-hover:text-[#2C5F2D] transition-colors">
                          {testimonial.name}
                        </h4>
                        <p className="text-sm text-[#E8A87C] font-medium">
                          {testimonial.role}
                        </p>
                      </div>
                    </div>
              ...
              testimonials.map((_, index) => (
                <motion.button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    currentIndex === index
                      ? 'bg-gradient-to-r from-[#2C5F2D] to-[#E8A87C] w-8'
        ...
        <motion.div
          ...
          className="mt-20 pt-10 border-t border-[#0A2647]/10"
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 lg:gap-8">
            {[
              ...
              >
                <div className="text-3xl md:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-[#0A2647] to-[#2C5F2D] bg-clip-text text-transparent mb-2">
                  {stat.value}
                </div>
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-full shadow-sm border border-[#0A2647]/10">
            <div className="flex">
              {[1, 2, 3, 4, 5].map((star) => (
